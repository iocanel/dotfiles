...
kitty --title "Auxiliary Terminal" -e bash -c "source '$SCRIPT_DIR/git-lib.sh'; mkdir -p $WORKSPACE; cd $WORKSPACE; if [ ! -d ${REPOSITORY_NAME} ]; then smart_clone '$HOST' '$ORGANIZATION' '$REPOSITORY_NAME' '$REPOSITORY_NAME'; fi; cd $REPOSITORY_NAME; exec fish"
popd